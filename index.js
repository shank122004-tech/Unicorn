...
async function handlePaymentSuccess(orderId, paymentType, pending, paymentData, pendingRef, webhookId, webhookStartTs) {
  ...
  await db.runTransaction(async (t) => {
    ...
    // ── POOL BOOKING ──────────────────────────────────────────────────
    else if (paymentType === "pool" || paymentType === "pool_booking") {
      const poolBookingRef = db.collection("pool_bookings").doc(orderId);

      t.set(poolBookingRef, {
        ...pending,
        ...paymentMeta,
        bookingId      : orderId,
        orderId        : orderId,
      ...
      // Update pool_slots member count if slotId is provided
      if (pending.slotId) {
        const slotRef = db.collection("pool_slots").doc(pending.slotId);
        const slotSnap = await t.get(slotRef);
        if (slotSnap.exists) {
          const slotData = slotSnap.data();
          const maxMembers = slotData.maxMembers || 50;
          const currentMembers = (slotData.currentMembers || 0) + 1;
          t.update(slotRef, {
            currentMembers : currentMembers,
            status         : currentMembers >= maxMembers ? "full" : "available",
            updatedAt      : now,
          });
        }
      }

      // Write owner_payments record for pool booking
      if (pending.ownerId) {
        const ownerPayRef = db.collection("owner_payments").doc(`${orderId}_pool_owner`);
        t.set(ownerPayRef, {
          orderId,
          ownerId        : pending.ownerId,
      ...
      console.log(`✅ [TRANSACTION] Pool booking confirmed: ${orderId} | slot: ${pending.slotId}`);
    }
