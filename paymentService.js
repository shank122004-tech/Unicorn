...
function _handlePaymentSuccess(orderId, paymentType, data) {
  ...
  // PATCH: Mark slot as confirmed when booking payment succeeds
  if (data && data.bookingId && data.groundId && data.slotTime && data.date) {
    if (typeof window.markSlotAsConfirmed === 'function') {
      window.markSlotAsConfirmed(data).catch(err => {
