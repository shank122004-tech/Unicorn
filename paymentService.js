  ...
  function patchOwnerEarnings() {
    const _earningsFn = async function (container) {
      ...
      try {
        ...
        // ── Fetch bookings + pool bookings + tournament entries in parallel ──────────────
        const [bookingSnap, poolBookingSnap, tournamentSnap] = await Promise.all([
        ...
        (poolBookingSnap.docs || []).forEach(doc => {
          ...
          if (p.date >= weekStart)  pWeekE  += amt;
          if (p.date >= monthStart) pMonthE += amt;
          poolBookings.push(p);
        });
        ...
        const totalE  = bTotalE  + pTotalE + tTotalE;
        const todayE  = bTodayE  + pTodayE + tTodayE;
        const weekE   = bWeekE   + pWeekE  + tWeekE;
        const monthE  = bMonthE  + pMonthE + tMonthE;
        ...
        // ── Build pool booking rows ──────────────────────────────────────
        const poolBookingRows = poolBookings.slice(0, 30).map(p => `
          <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 0;border-bottom:1px solid var(--gray-100,#f3f4f6);">
            <div>
              <div style="font-weight:600;font-size:14px;">${_esc(p.poolName || 'Pool Booking')}</div>
              <div style="font-size:12px;color:var(--gray-500);">${p.date} • ${p.time || p.slotTime || ''} ${p.members ? '• '+p.members+' members' : ''}</div>
              <div style="font-size:11px;color:var(--gray-400);">ID: ${p.bookingId || ''}</div>
            </div>
            <div style="text-align:right;">
              <div style="font-weight:700;color:#0ea5e9;font-size:16px;">${fmt(p.ownerAmount || 0)}</div>
              <div style="font-size:11px;color:var(--gray-400);">your share</div>
            </div>
          </div>`).join('');
        ...
        container.innerHTML = `
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:10px;margin-bottom:20px;">
            <div style="background:var(--bg-secondary,#f9fafb);border-radius:12px;padding:14px;text-align:center;border:1px solid #e5e7eb;">
              ...
              <div style="font-size:10px;color:var(--gray-400);">${bookings.length} grounds</div>
            </div>
            <div style="background:var(--bg-secondary,#f9fafb);border-radius:12px;padding:14px;text-align:center;border:1px solid #e5e7eb;">
              <div style="font-size:10px;color:var(--gray-500);margin-bottom:4px;">Pool Earnings</div>
              <div style="font-size:16px;font-weight:700;color:#0ea5e9;">${fmt(pTotalE)}</div>
              <div style="font-size:10px;color:var(--gray-400);">${poolBookings.length} pools</div>
            </div>
            <div style="background:var(--bg-secondary,#f9fafb);border-radius:12px;padding:14px;text-align:center;border:1px solid #e5e7eb;">
          ...
          <!-- POOL BOOKING HISTORY -->
          <div style="margin-bottom:20px;">
            <div style="font-weight:700;font-size:15px;margin-bottom:12px;">
              <i class="fas fa-water" style="color:#0ea5e9;"></i> Pool Booking Earnings
              <span style="font-weight:400;font-size:12px;color:var(--gray-500);margin-left:8px;">(${poolBookings.length} bookings)</span>
            </div>
            ${poolBookings.length === 0
              ? '<div style="text-align:center;padding:24px;color:var(--gray-400);">No pool bookings yet</div>'
              : poolBookingRows}
          </div>
