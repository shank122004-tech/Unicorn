...
(function () {
  ...
  function patchOwnerEarnings() {
    // ── [N5] Owner Earnings — defer to app.js loadOwnerEarnings (the source of truth) ──
    // app.js's loadOwnerEarnings already contains the full, correct implementation.
    // We expose it on window so both the tab switcher in app.js and any external
    // callers from paymentService.js call the same code path.
    async function _earningsBridge(container) {
      // Wait up to 3 s for app.js to finish declaring loadOwnerEarnings
      let attempts = 0;
