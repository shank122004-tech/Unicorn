/* all_patches_combined.js — extracted from sportobook_master.js
 */
...
(function () {
    ...
    function setActiveCity(cityRaw) {
        ...
        /* Flag so that after reload we know to land on home page */
        localStorage.setItem('bmg_city_just_changed', '1');
        updateCityBar();
        _triggerCityReload();
    }

    /* Central reload: saves city to localStorage then does a full page reload
       so every section (grounds, pools, deals, featured) refreshes cleanly. */
    function _triggerCityReload() {
        /* Show a loading overlay so the user sees something happening */
        var overlay = document.getElementById('bmg-city-reload-overlay');
        if (!overlay) {
            overlay = document.createElement('div');
            overlay.id = 'bmg-city-reload-overlay';
            ...
            overlay.innerHTML =
                '<div style="width:44px;height:44px;border:4px solid #e5e7eb;' +
                'border-top-color:#4f46e5;border-radius:50%;' +
                'animation:bmg-spin 0.7s linear infinite"></div>' +
                '<div style="font-size:15px;font-weight:600;color:#1a1a2e">' +
                'Loading ' + (activeCityDisplay || 'All Cities') + '\u2026</div>' +
                '<style>@keyframes bmg-spin{to{transform:rotate(360deg)}}</style>';
            document.body.appendChild(overlay);
        }

        /* Short delay so the overlay is visible before the reload fires */
        setTimeout(function () {
            location.reload();
        }, 350);
    }
...
(function () {
    ...
    function installHardenedPoolLoader() {
        ...
        function _fallbackCard(pool) {
            ...
            var displayPrice = pool.basePricePerMember || pool.pricePerSession || 0;
            var displayName  = pool.poolName || pool.name || 'Swimming Pool';
            card.innerHTML =
              '<div class="bmg-pool-card-img-placeholder">🏊</div>' +
              '<div class="bmg-pool-card-body">' +
                '<div class="bmg-pool-card-name">' + _esc(displayName) + '</div>' +
                ...
                '<div class="bmg-pool-card-price-row">' +
                  '<span class="bmg-pool-card-price">₹' + displayPrice + '</span>' +
                  '<span class="bmg-pool-card-price-sub">/session</span>' +
                '</div>' +
              '</div>';
            ...
            if (!activeCity) {
                ...
                try {
                    ...
                    for (var i = 0; i < snap.docs.length; i++) {
                        ...
                        data.poolName           = data.poolName           || data.name           || 'Swimming Pool';
                        data.basePricePerMember = data.basePricePerMember || data.pricePerSession || 0;
                        data.maxMembersPerSlot  = data.maxMembersPerSlot  || data.capacityPerSession || 50;
                ...
                try {
                    ...
                    snap1.forEach(function (d) {
                        ...
                        if (cityMatch(data, activeCity)) {
                            ...
                            /* Normalise field names: owner form saves `name` and
                               `pricePerSession`; buildPoolHomeCard reads `poolName`
                               and `basePricePerMember`.  Bridge the gap here. */
                            data.poolName           = data.poolName           || data.name           || 'Swimming Pool';
                            data.basePricePerMember = data.basePricePerMember || data.pricePerSession || 0;
                            data.maxMembersPerSlot  = data.maxMembersPerSlot  || data.capacityPerSession || 50;
...
(function () {
    ...
    async function verifyPoolEntryQR(qrObject) {
        ...
        // Time window check — allow entry 15 min before slot start until slot end
        var poolTimeRaw2 = booking.time || booking.slotTime || qrObject.slot || '';
        if (poolTimeRaw2) {
            try {
                var nowP = new Date();
                var timeParts2 = poolTimeRaw2.split('-');
                var startPart2 = timeParts2[0].trim();
                var endPart2   = timeParts2.length >= 2 ? timeParts2[timeParts2.length - 1].trim() : startPart2;
                function parseHMP(str) {
                    var mp = str.match(/(\d{1,2}):(\d{2})\s*(AM|PM)?/i);
                    if (!mp) return null;
                    var hP = parseInt(mp[1], 10), mP = parseInt(mp[2], 10);
                    if (mp[3]) {
                        if (mp[3].toUpperCase() === 'PM' && hP !== 12) hP += 12;
                        if (mp[3].toUpperCase() === 'AM' && hP === 12) hP = 0;
                    }
                    return { h: hP, m: mP };
                }
                var startHMP = parseHMP(startPart2);
                var endHMP   = parseHMP(endPart2);
                var baseDate2 = booking.date || today;
                if (startHMP) {
                    var slotStartP = new Date(baseDate2 + 'T00:00:00');
                    slotStartP.setHours(startHMP.h, startHMP.m, 0, 0);
                    var entryWindowStartP = new Date(slotStartP);
                    entryWindowStartP.setMinutes(entryWindowStartP.getMinutes() - 15);
                    if (nowP < entryWindowStartP) {
                        var minsWait = Math.ceil((entryWindowStartP - nowP) / 60000);
                        throw new Error('Pool entry will be allowed at ' + entryWindowStartP.toLocaleTimeString('en-IN', {hour:'2-digit',minute:'2-digit'}) + '. Please wait ' + minsWait + ' minute(s).');
                    }
                }
                if (endHMP) {
                    var slotEndP = new Date(baseDate2 + 'T00:00:00');
                    slotEndP.setHours(endHMP.h, endHMP.m, 0, 0);
                    if (nowP > slotEndP) {
                        throw new Error('Pool entry window closed at ' + slotEndP.toLocaleTimeString('en-IN', {hour:'2-digit',minute:'2-digit'}) + '.');
                    }
                }
            } catch(eP) {
                if (eP.message && (eP.message.indexOf('Pool entry will') !== -1 || eP.message.indexOf('Pool entry window') !== -1)) throw eP;
            }
        }
...
(function () {
  ...
  async function loadOwnerPools(container) {
    ...
    try {
            ...
            <div style="background:#fff;border-radius:16px;overflow:hidden;
              ...
              <div style="padding:12px 14px;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px;">
                  <div style="background:#f0f9ff;border-radius:10px;padding:7px 10px;">
                    <div style="font-size:9px;color:#94a3b8;font-weight:700;text-transform:uppercase;letter-spacing:.4px;">Opens</div>
                    <div style="font-size:13px;font-weight:800;color:#0369a1;">${esc(pool.openTime || '06:00')}</div>
                  </div>
                  <div style="background:#f0f9ff;border-radius:10px;padding:7px 10px;">
                    <div style="font-size:9px;color:#94a3b8;font-weight:700;text-transform:uppercase;letter-spacing:.4px;">Closes</div>
                    <div style="font-size:13px;font-weight:800;color:#0369a1;">${esc(pool.closeTime || '21:00')}</div>
                  </div>
                  <div style="background:#f0f9ff;border-radius:10px;padding:7px 10px;">
                    <div style="font-size:9px;color:#94a3b8;font-weight:700;text-transform:uppercase;letter-spacing:.4px;">Capacity</div>
                    <div style="font-size:13px;font-weight:800;color:#0369a1;">${esc(pool.capacityPerSession || 30)} /session</div>
                  </div>
                  <div style="background:#f0f9ff;border-radius:10px;padding:7px 10px;">
                    <div style="font-size:9px;color:#94a3b8;font-weight:700;text-transform:uppercase;letter-spacing:.4px;">Length</div>
                    <div style="font-size:13px;font-weight:800;color:#0369a1;">${pool.poolLength ? esc(pool.poolLength)+'m' : '—'}</div>
                  </div>
                </div>
                <button class="bmgp-edit-pool-btn" data-pool-id="${esc(pool.id)}"
                  style="width:100%;padding:11px;background:linear-gradient(135deg,#0369a1,#0ea5e9);
                         color:#fff;border:none;border-radius:12px;font-size:14px;font-weight:700;
                         cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;">
                  <i class="fas fa-edit"></i> Edit Pool Details
                </button>
              </div>
            </div>`;
        });
      ...
      /* Bind edit buttons */
      document.querySelectorAll('.bmgp-edit-pool-btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
          var pid = btn.getAttribute('data-pool-id');
          if (pid) showBmgpEditPoolModal(pid);
        });
      });
  ...
  window.showAddPoolModal = function() { showAddPoolModal(); };
  ...
  function createBmgpEditPoolModalIfNeeded() {
    if (gid('bmgp-edit-pool-modal')) return;
    var modal = document.createElement('div');
    ...
    modal.innerHTML = `
      <div class="modal-content" style="max-width:520px;max-height:92vh;overflow-y:auto;border-radius:20px;padding:0;">
        <div style="display:flex;align-items:center;justify-content:space-between;
                    padding:18px 18px 14px;border-bottom:1px solid #f0f4ff;
                    position:sticky;top:0;background:#fff;z-index:10;border-radius:20px 20px 0 0;">
          <div style="font-size:17px;font-weight:800;color:#0f1f5c;display:flex;align-items:center;gap:8px;">
            <i class="fas fa-edit" style="color:#0ea5e9;"></i> Edit Pool
          </div>
        ...
        <div style="padding:18px 18px 24px;" id="bmgp-edit-pool-body">
          <input type="hidden" id="bmgp-ep-id">

          <!-- Name & City -->
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;">
            <div>
              <label style="font-size:12px;font-weight:700;color:#374151;display:block;margin-bottom:5px;"><i class="fas fa-tag"></i> Pool Name *</label>
              <input id="bmgp-ep-name" type="text" maxlength="100" placeholder="Pool name"
                style="width:100%;padding:11px 13px;border:1.5px solid #e2e8f0;border-radius:12px;font-size:14px;box-sizing:border-box;outline:none;">
            </div>
            <div>
              <label style="font-size:12px;font-weight:700;color:#374151;display:block;margin-bottom:5px;"><i class="fas fa-city"></i> City *</label>
              <input id="bmgp-ep-city" type="text" placeholder="City"
                style="width:100%;padding:11px 13px;border:1.5px solid #e2e8f0;border-radius:12px;font-size:14px;box-sizing:border-box;outline:none;">
            </div>
          </div>
          ...
          <div style="margin-bottom:14px;">
            <label style="font-size:12px;font-weight:700;color:#374151;display:block;margin-bottom:5px;"><i class="fas fa-map-marker-alt"></i> Full Address *</label>
            <textarea id="bmgp-ep-address" rows="2" placeholder="Street, Area, City, PIN"
              style="width:100%;padding:11px 13px;border:1.5px solid #e2e8f0;border-radius:12px;font-size:14px;box-sizing:border-box;outline:none;resize:none;"></textarea>
          </div>

          <!-- Description -->
          <div style="margin-bottom:14px;">
            <label style="font-size:12px;font-weight:700;color:#374151;display:block;margin-bottom:5px;"><i class="fas fa-align-left"></i> Description</label>
            <textarea id="bmgp-ep-description" rows="2" placeholder="Describe your pool…"
              style="width:100%;padding:11px 13px;border:1.5px solid #e2e8f0;border-radius:12px;font-size:14px;box-sizing:border-box;outline:none;resize:none;"></textarea>
          </div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;">
            <div>
              <label style="font-size:12px;font-weight:700;color:#374151;display:block;margin-bottom:5px;"><i class="fas fa-rupee-sign"></i> Price/Session (₹) *</label>
              <input id="bmgp-ep-price" type="number" min="1" placeholder="200"
                style="width:100%;padding:11px 13px;border:1.5px solid #e2e8f0;border-radius:12px;font-size:14px;box-sizing:border-box;outline:none;">
            </div>
            <div>
              <label style="font-size:12px;font-weight:700;color:#374151;display:block;margin-bottom:5px;"><i class="fas fa-phone"></i> Contact Number *</label>
              <input id="bmgp-ep-phone" type="tel" maxlength="10" placeholder="10-digit number"
                style="width:100%;padding:11px 13px;border:1.5px solid #e2e8f0;border-radius:12px;font-size:14px;box-sizing:border-box;outline:none;">
            </div>
          </div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;">
            <div>
              <label style="font-size:12px;font-weight:700;color:#374151;display:block;margin-bottom:5px;"><i class="fas fa-clock"></i> Opens At *</label>
              <input id="bmgp-ep-open" type="time"
                style="width:100%;padding:11px 13px;border:1.5px solid #e2e8f0;border-radius:12px;font-size:14px;box-sizing:border-box;outline:none;">
            </div>
            <div>
              <label style="font-size:12px;font-weight:700;color:#374151;display:block;margin-bottom:5px;"><i class="fas fa-clock"></i> Closes At *</label>
              <input id="bmgp-ep-close" type="time"
                style="width:100%;padding:11px 13px;border:1.5px solid #e2e8f0;border-radius:12px;font-size:14px;box-sizing:border-box;outline:none;">
            </div>
          </div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;">
            <div>
              <label style="font-size:12px;font-weight:700;color:#374151;display:block;margin-bottom:5px;"><i class="fas fa-users"></i> Capacity/Session *</label>
              <input id="bmgp-ep-capacity" type="number" min="1" placeholder="30"
                style="width:100%;padding:11px 13px;border:1.5px solid #e2e8f0;border-radius:12px;font-size:14px;box-sizing:border-box;outline:none;">
            </div>
            <div>
              <label style="font-size:12px;font-weight:700;color:#374151;display:block;margin-bottom:5px;"><i class="fas fa-ruler"></i> Pool Length (m)</label>
              <input id="bmgp-ep-length" type="number" min="0" placeholder="25"
                style="width:100%;padding:11px 13px;border:1.5px solid #e2e8f0;border-radius:12px;font-size:14px;box-sizing:border-box;outline:none;">
            </div>
          </div>
          ...
          <!-- Status -->
          <div style="margin-bottom:14px;">
            <label style="font-size:12px;font-weight:700;color:#374151;display:block;margin-bottom:5px;"><i class="fas fa-toggle-on"></i> Pool Status</label>
            <select id="bmgp-ep-status"
              style="width:100%;padding:11px 13px;border:1.5px solid #e2e8f0;border-radius:12px;font-size:14px;box-sizing:border-box;outline:none;background:#fff;">
          ...
          <!-- Amenities -->
          <div style="margin-bottom:14px;">
            <label style="font-size:12px;font-weight:700;color:#374151;display:block;margin-bottom:8px;"><i class="fas fa-star"></i> Amenities</label>
            <div style="display:grid;grid-template-columns:1fr 1fr;background:#f8fafc;border-radius:12px;padding:10px 14px;">
              ${['Changing Rooms','Lockers','Coach Available','Parking','Cafe / Canteen','Lifeguard','Kids Pool','Heated Pool'].map(function(a){
                var key = a.toLowerCase().replace(/[^a-z]/g,'_');
                return '<label style="display:flex;align-items:center;gap:6px;font-size:13px;cursor:pointer;padding:5px 0;"><input type="checkbox" class="bmgp-ep-amenity" value="'+a+'" id="bmgp-ep-am-'+key+'" style="width:15px;height:15px;accent-color:#0ea5e9;cursor:pointer;"> '+a+'</label>';
              }).join('')}
            </div>
          </div>
          ...
          <div style="margin-bottom:18px;">
            <label style="font-size:12px;font-weight:700;color:#374151;display:block;margin-bottom:8px;"><i class="fas fa-images"></i> Pool Photos</label>
            <div id="bmgp-ep-photo-preview" style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:10px;"></div>
            <label style="display:flex;align-items:center;justify-content:center;gap:8px;
                          padding:11px;border:2px dashed #bae6fd;border-radius:12px;cursor:pointer;
                          font-size:13px;font-weight:600;color:#0ea5e9;background:#f0f9ff;">
              <i class="fas fa-camera"></i> Add More Photos
              <input type="file" id="bmgp-ep-photo-input" accept="image/jpeg,image/png,image/webp" multiple style="display:none;">
            </label>
            <p style="font-size:11px;color:#94a3b8;margin:6px 0 0;">Max 6 photos. First photo is the cover image.</p>
          </div>

          <!-- Save -->
          <button id="bmgp-ep-save-btn"
            style="width:100%;padding:14px;background:linear-gradient(135deg,#0369a1,#0ea5e9);
                   color:#fff;border:none;border-radius:14px;font-size:15px;font-weight:800;cursor:pointer;">
  ...
  window.showBmgpEditPoolModal = function(poolId) { showBmgpEditPoolModal(poolId); };

  async function showBmgpEditPoolModal(poolId) {
    spinStart('Loading pool details…');
    try {
      var doc = await db.collection('swimming_pools').doc(poolId).get();
      spinStop();
      if (!doc.exists) { toast('Pool not found', 'error'); return; }
      var p = Object.assign({ id: doc.id }, doc.data());

      createBmgpEditPoolModalIfNeeded();
      var modal = gid('bmgp-edit-pool-modal');
      if (!modal) return;

      /* Fill fields */
      gid('bmgp-ep-id').value          = p.id;
      gid('bmgp-ep-name').value         = p.name || '';
      gid('bmgp-ep-city').value         = p.city || '';
      gid('bmgp-ep-address').value      = p.address || '';
      gid('bmgp-ep-description').value  = p.description || '';
      gid('bmgp-ep-price').value        = p.pricePerSession || 0;
      gid('bmgp-ep-phone').value        = p.contactPhone || '';
      gid('bmgp-ep-open').value         = p.openTime  || '06:00';
      gid('bmgp-ep-close').value        = p.closeTime || '21:00';
      gid('bmgp-ep-capacity').value     = p.capacityPerSession || 30;
      gid('bmgp-ep-length').value       = p.poolLength || '';
      gid('bmgp-ep-status').value       = p.status || 'active';

      /* Amenities */
      document.querySelectorAll('.bmgp-ep-amenity').forEach(function (cb) {
        cb.checked = (p.amenities || []).indexOf(cb.value) !== -1;
      });

      /* Photos */
      _bmgpEditPhotos = p.images ? p.images.slice() : [];
      renderBmgpEditPhotoGrid();

      /* Photo input */
      var fi = gid('bmgp-ep-photo-input');
      if (fi) {
        var newFi = fi.cloneNode(true);
        fi.parentNode.replaceChild(newFi, fi);
        newFi.addEventListener('change', async function () {
          var files = Array.from(newFi.files);
          if (!files.length) return;
          if (_bmgpEditPhotos.length >= 6) { toast('Max 6 photos allowed', 'error'); return; }
          spinStart('Uploading photos…');
          try {
            var toUpload = files.slice(0, 6 - _bmgpEditPhotos.length);
            for (var i = 0; i < toUpload.length; i++) {
              ...
              _bmgpEditPhotos.push(url);
            }
            renderBmgpEditPhotoGrid();
            spinStop();
            toast(toUpload.length + ' photo(s) uploaded ✅', 'success');
          } catch(err) {
            spinStop();
            toast('Upload failed: ' + err.message, 'error');
          }
      ...
      /* Close */
      var closeBtn = gid('bmgp-edit-pool-close');
      if (closeBtn) {
        var newClose = closeBtn.cloneNode(true);
        closeBtn.parentNode.replaceChild(newClose, closeBtn);
        gid('bmgp-edit-pool-close').addEventListener('click', function () { modal.classList.remove('active'); });
      }
      modal.addEventListener('click', function (e) {
        if (e.target === modal) modal.classList.remove('active');
      });
      ...
      /* Save */
      var saveBtn = gid('bmgp-ep-save-btn');
      if (saveBtn) {
        var newSave = saveBtn.cloneNode(true);
        saveBtn.parentNode.replaceChild(newSave, saveBtn);
        gid('bmgp-ep-save-btn').addEventListener('click', function () { saveBmgpPoolEdits(); });
      }

      modal.classList.add('active');
    } catch(err) {
      spinStop();
      console.error('[EditPool]', err);
      toast('Failed to load pool: ' + err.message, 'error');
    }
  }

  function renderBmgpEditPhotoGrid() {
    var preview = gid('bmgp-ep-photo-preview');
    if (!preview) return;
    if (_bmgpEditPhotos.length === 0) {
      preview.innerHTML = '<p style="font-size:12px;color:#94a3b8;margin:0;">No photos yet</p>';
      return;
    }
    preview.innerHTML = _bmgpEditPhotos.map(function (url, i) {
      return '<div style="position:relative;display:inline-block;">' +
        '<img src="' + esc(url) + '" style="width:72px;height:72px;object-fit:cover;border-radius:10px;border:2px solid #e2e8f0;">' +
        '<button onclick="bmgpRemoveEditPhoto(' + i + ')" style="position:absolute;top:-6px;right:-6px;background:#ef4444;color:#fff;border:none;border-radius:50%;width:20px;height:20px;font-size:12px;cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;line-height:1;">×</button>' +
        (i === 0 ? '<div style="position:absolute;bottom:2px;left:2px;background:rgba(3,105,161,.85);color:#fff;font-size:9px;font-weight:700;padding:1px 5px;border-radius:4px;">Cover</div>' : '') +
        '</div>';
    }).join('');
  }

  window.bmgpRemoveEditPhoto = function (index) {
    _bmgpEditPhotos.splice(index, 1);
    renderBmgpEditPhotoGrid();
  };

  async function saveBmgpPoolEdits() {
    var poolId      = (gid('bmgp-ep-id')?.value || '').trim();
    var name        = (gid('bmgp-ep-name')?.value || '').trim();
    var city        = (gid('bmgp-ep-city')?.value || '').trim();
    var address     = (gid('bmgp-ep-address')?.value || '').trim();
    var description = (gid('bmgp-ep-description')?.value || '').trim();
    var price       = parseInt(gid('bmgp-ep-price')?.value) || 0;
    var phone       = (gid('bmgp-ep-phone')?.value || '').trim();
    var openTime    = gid('bmgp-ep-open')?.value  || '06:00';
    var closeTime   = gid('bmgp-ep-close')?.value || '21:00';
    var capacity    = parseInt(gid('bmgp-ep-capacity')?.value) || 30;
    var poolLength  = parseInt(gid('bmgp-ep-length')?.value)   || 0;
    var status      = gid('bmgp-ep-status')?.value || 'active';
    ...
    if (!price || price < 1) { toast('Enter a valid price', 'error'); return; }
    if (openTime >= closeTime) { toast('Opening time must be before closing time', 'error'); return; }

    /* Amenities */
    var amenities = [];
    document.querySelectorAll('.bmgp-ep-amenity:checked').forEach(function (cb) {
      amenities.push(cb.value);
    });

    spinStart('Saving changes…');
    try {
      // Write BOTH field-name conventions so the booking page (basePricePerMember)
      // and the owner dashboard (pricePerSession) always stay in sync.
      await db.collection('swimming_pools').doc(poolId).update({
        name:               name,
        city:               city,
        cityLower:          city.toLowerCase(),
        address:            address,
        ...
        basePricePerMember: price,          // ← booking page reads this
        contactPhone:       phone,
        openTime:           openTime,
        closeTime:          closeTime,
        capacityPerSession: capacity,
        maxMembersPerSlot:  capacity,       // ← booking page reads this
        poolLength:         poolLength,
        amenities:          amenities,
        images:             _bmgpEditPhotos,
        status:             status,
        updatedAt:          firebase.firestore.FieldValue.serverTimestamp()
      });

      // Also push the new price + capacity into all future pool_slots so users
      // see the updated price immediately on the booking page without waiting
      // for ensurePoolSlotDocs to re-run.
      try {
        var today = new Date().toISOString().split('T')[0];
        var futureSlots = await db.collection('pool_slots')
          .where('poolId', '==', poolId)
          .where('date', '>=', today)
          .get();
        if (!futureSlots.empty) {
          var slotBatch = db.batch();
          futureSlots.forEach(function (slotDoc) {
            slotBatch.update(slotDoc.ref, {
              basePricePerMember: price,
              maxMembers:         capacity,
              updatedAt:          firebase.firestore.FieldValue.serverTimestamp()
            });
          });
          await slotBatch.commit();
          console.log('[EditPool] Updated', futureSlots.size, 'pool_slots with new price ₹' + price);
        }
      } catch (slotErr) {
        console.warn('[EditPool] pool_slots price sync failed (non-critical):', slotErr.message);
      }
      ...
      spinStop();
      console.error('[EditPool] save error:', err);
  ...
  // Listen for openAddPool event dispatched from app.js loadOwnerPools button
  window.addEventListener('bmg:openAddPool', function() {
    showAddPoolModal();
  });
...
`;
  ...
  async function pspOwnerPayouts(container) {
    ...
    try {
      ...
      // ── Earnings: only count bookings NOT yet paid out ──────────────────
      // Skip any booking/pool-booking whose payoutStatus is already 'payout_done'
      // so that confirmed-paid bookings never appear as "available" again.
      // FIXED: always derive ownerAmount — never skip when field is 0
      let totalEarned = 0;
      bookSnap.docs.forEach(d => {
        const b = d.data();
        if (b.payoutStatus === 'payout_done') return; // already paid — exclude
        totalEarned += Number(b.ownerAmount) || Math.floor(Number(b.amount || 0) * 0.9);
      });
      poolSnap.docs.forEach(d => {
        const p = d.data();
        if (p.payoutStatus === 'payout_done') return; // already paid — exclude
        totalEarned += Number(p.ownerAmount) || Math.floor(Number(p.amount || 0) * 0.9);
      });

      // ── Received: only paid payout_requests (owner explicitly requested & got paid) ─────────
      // NOTE: owner_payments records are written by the Cloud Function as internal payment
      // receipts and do NOT represent money transferred to the owner. Counting them as
      // "received" incorrectly zeroes out the available balance.
      let totalReceived = 0;
      const receivedItems = [];
      const seenIds = new Set();

      // owner_payments: show in history only, do NOT subtract from available
      paymentSnap.docs.forEach(d => {
        seenIds.add(d.id);
        const p = d.data();
        // history only — not added to totalReceived
      ...
      // ── In-flight: amounts locked inside pending/approved payout requests ─
      // These haven't been received yet but are already "spoken for", so subtract
      // them from the available balance so owner can't request the same money twice.
      let inFlight = 0;
      payoutSnap.docs.forEach(d => {
        const p = d.data();
      ...
      // Available = unpaid earnings − amounts already in active payout requests
      // (totalReceived handles the already-paid portion via owner_payments / paid requests)
...
<div class="psp-page">
  ...
  <div class="psp-hero">
    <div>
      ...
      <div style="font-size:11px;opacity:.65;margin-top:4px;">Earned ${_fmt(totalEarned)}${inFlight > 0 ? ` · In review ${_fmt(inFlight)}` : `${totalReceived > 0 ? ` · Received ${_fmt(totalReceived)}` : ''}`}</div>
    </div>
  ...
  ${inFlight > 0 ? `
  <div style="background:#fffbeb;border:1.5px solid #fde68a;border-radius:12px;padding:11px 14px;margin-bottom:14px;font-size:12px;color:#92400e;display:flex;align-items:center;gap:8px;">
    <i class="fas fa-clock"></i>
    <span><b>${_fmt(inFlight)}</b> is already under a pending payout request.</span>
  </div>` : ''}
  ...
  <div class="psp-modal-actions">
    ...
    document.getElementById('psp-apply-confirm').addEventListener('click', async () => {
      ...
      try {
        ...
        // Collect actual source bookings that are not yet paid out
        // so markPayoutAsPaid can find and update them correctly
        ...
        const sources = [];
        let accumulated = 0;
        const requestedAmt = Math.round(amtVal);

        // Ground bookings first (FIFO)
        for (const d of bSnap.docs) {
          if (accumulated >= requestedAmt) break;
          const b = d.data();
          if (b.payoutStatus === 'payout_done' || b.payoutStatus === 'payout_pending') continue;
          sources.push({ type: 'ground_booking', id: d.id, amount: b.ownerAmount || 0 });
          accumulated += b.ownerAmount || 0;
        }
        // Pool bookings next
        for (const d of pSnap.docs) {
          if (accumulated >= requestedAmt) break;
          const p = d.data();
          if (p.payoutStatus === 'payout_done' || p.payoutStatus === 'payout_pending') continue;
          sources.push({ type: 'pool_booking', id: d.id, amount: p.ownerAmount || p.amount || 0 });
          accumulated += p.ownerAmount || p.amount || 0;
        }

        const docRef = await db.collection('payout_requests').add({
          requestId: reqId,
          ownerId: cu.uid,
          ...
          note: note,
          status: 'pending',
          sources: sources.map(s => ({ type: s.type, id: s.id, amount: s.amount })),
          bookingIds: sources.filter(s=>s.type==='ground_booking').map(s=>s.id),
          createdAt: now,
          updatedAt: now,
        });

        // Mark source docs as payout_pending so they're excluded from future requests
        if (sources.length > 0) {
          const batch = db.batch();
          sources.forEach(s => {
            const col = s.type === 'pool_booking' ? 'pool_bookings' : 'bookings';
            batch.update(db.collection(col).doc(s.id), {
              payoutStatus: 'payout_pending',
              payoutRequestId: reqId,
              payoutRequestDocId: docRef.id,
              updatedAt: now,
            });
          });
          await batch.commit();
  ...
  <div class="psp-modal-actions">
    ...
    document.getElementById('psp-pay-confirm').addEventListener('click', async () => {
      ...
      try {
        ...
        // Fetch the payout request to get sources (booking IDs to mark done)
        const payoutDoc = await db.collection('payout_requests').doc(docId).get();
        const payoutData = payoutDoc.exists ? payoutDoc.data() : {};
        const sources = payoutData.sources || [];
        ...
        /* 2. Mark all source booking/pool-booking docs as payout_done
              so they never appear in the owner's "available" balance again */
        if (sources.length > 0) {
          const batch = db.batch();
          sources.forEach(s => {
            const col = s.type === 'pool_booking' ? 'pool_bookings' : 'bookings';
          ...
          // Legacy: also handle old requests that stored bookingIds instead of sources
          const legacyIds = payoutData.bookingIds || [];
          for (const bid of legacyIds) {
            if (!sources.find(s => s.id === bid)) {
        ...
        /* 3. Write to owner_payments so owner's earnings section shows it */
