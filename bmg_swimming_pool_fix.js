...
(function () {
  ...
  async function showPoolEntryPass(bookingId) {
    ...
    try {
      ...
      if (!snap1.empty) {
        booking = snap1.docs[0].data();
      } else {
        var snap2 = await db.collection('bookings')
          .where('bookingId', '==', bookingId).limit(1).get()
          .catch(function () { return { empty: true }; });
        if (!snap2.empty) booking = snap2.docs[0].data();
      }
      ...
      var isConfirmed = booking.bookingStatus === 'confirmed' || booking.status === 'confirmed';
      if (!isConfirmed) {
        if (typeof window.hideLoading === 'function') window.hideLoading();
        ...
        '<div class="bmg-pool-entry-pass">' +
          ...
          '<div class="bmg-pool-pass-header">' +
            ...
            '<div class="bmg-pool-pass-header-content">' +
              ...
              '<div class="bmg-pool-pass-subtitle">Show this at the pool entrance</div>' +
            '</div>' +
          '</div>' +
  ...
  function patchLoadUserBookings() {
    ...
    window.loadUserBookings = async function (status) {
      /* Run original first — it populates the container */
      await original.apply(this, arguments);

      /* Now append pool bookings */
      var user = window.currentUser;
      if (!user) return;
      ...
      try {
        ...
        var filtered = poolBookings.filter(function (b) {
          var bDate       = b.date || '';
          var bStatus     = b.bookingStatus || b.status || '';
          var isPast      = bDate < today;
          var isConfirmed = bStatus === 'confirmed';
          var isCompleted = bStatus === 'completed';
          var isCancelled = bStatus === 'cancelled';

          if (status === 'upcoming') return bDate >= today && isConfirmed;
        ...
        filtered.forEach(function (b) {
          var bStatus = b.bookingStatus || b.status || 'pending';
          var scMap = {
          ...
          var sc = scMap[bStatus] || { icon: 'fa-circle', label: bStatus, cls: 'pending' };

          var memberCount = b.memberCount || 1;
          var perPer      = b.pricePerMember || b.amount || 0;
          var total       = b.amount || (perPer * memberCount);

          var dateLabel = b.date || '';
          try {
            dateLabel = new Date(b.date).toLocaleDateString('en-IN', {
              weekday: 'long', day: 'numeric', month: 'long', year: 'numeric'
            });
          } catch (e) {}

          var isPassed = b.date < today && bStatus !== 'completed';

          html +=
            '<div class="bk-card bk-card--' + sc.cls + ' bmg-pool-bk-card">' +
              '<div class="bmg-pool-bk-wave-strip"></div>' +
              '<div class="bk-card__header">' +
                '<div class="bk-card__sport-badge">🏊</div>' +
                '<div class="bk-card__title-block">' +
                  '<div class="bk-card__ground-name">' + _esc(b.poolName || 'Swimming Pool') + '</div>' +
                  '<div class="bk-card__venue-name">' +
                    '<span class="bmg-pool-bk-member-chip">' +
              ...
              '<div class="bk-card__details">' +
                '<div class="bk-card__detail-row">' +
                  ...
                  '<span class="bk-card__detail-text">' + _esc(dateLabel) + '</span>' +
                '</div>' +
                '<div class="bk-card__detail-row">' +
                  '<span class="bk-card__detail-icon"><i class="fas fa-clock"></i></span>' +
                  '<span class="bk-card__detail-text">' + _esc(b.slotTime || 'Time TBD') + '</span>' +
                '</div>' +
                '<div class="bk-card__detail-row">' +
                  '<span class="bk-card__detail-icon"><i class="fas fa-map-marker-alt"></i></span>' +
                  '<span class="bk-card__detail-text">' + _esc(b.poolAddress || 'Address not available') + '</span>' +
                '</div>' +
                '<div class="bk-card__detail-row">' +
                  '<span class="bk-card__detail-icon"><i class="fas fa-rupee-sign"></i></span>' +
                  '<span class="bk-card__detail-text bk-card__amount">' +
                    '₹' + total +
                    (memberCount > 1 ? ' <span style="font-size:11px;color:#64748b">(₹' + perPer + '/person)</span>' : '') +
                  '</span>' +
                '</div>' +
              '</div>' +
              ...
              '<div class="bk-card__footer">' +
                '<span class="bk-card__booking-id"><i class="fas fa-hashtag"></i> ' + _esc((b.bookingId || 'N/A').slice(-10)) + '</span>' +
                (bStatus === 'confirmed'
                  ? '<button class="bk-card__pass-btn bmg-pool-pass-btn-trigger" data-booking-id="' + _esc(b.bookingId) + '">' +
                      '<i class="fas fa-swimming-pool"></i> Pool Pass' +
                    '</button>'
                  : '') +
                (bStatus === 'completed' ? '<span class="bk-card__done-chip"><i class="fas fa-check-double"></i> Completed</span>' : '') +
                (bStatus === 'cancelled' ? '<span class="bk-card__cancel-chip"><i class="fas fa-ban"></i> Cancelled</span>' : '') +
              '</div>' +
            '</div>';
        });

        /* Prepend pool cards (most likely upcoming) */
        var existingContent = container.innerHTML;
        container.innerHTML = html + existingContent;

        /* Wire pool pass buttons */
        container.querySelectorAll('.bmg-pool-pass-btn-trigger').forEach(function (btn) {
          btn.addEventListener('click', function () {
            showPoolEntryPass(btn.getAttribute('data-booking-id'));
          });
        });

      } catch (err) {
        console.warn('[pool-fix v6+] loadUserBookings pool patch error:', err.message);
      }
    };

    window.loadUserBookings._poolPatched = true;
    console.log('[pool-fix v6+] loadUserBookings patched — pool bookings now visible in My Bookings');
  }
  ...
  window.addEventListener('bmg:paymentConfirmed', function (e) {
    ...
    /* Short delay for webhook to write confirmed status */
    setTimeout(function () {
      showPoolEntryPass(booking.bookingId);
    }, 2000);
  });
...
/* ════════════════════════════════════════════════════════════════════
 *  PART 6 — Fix pool slot currentMembers + status after payment
 *  app.js only adds +1 regardless of memberCount booked.
 *  We recalculate from all confirmed bookings and write the correct
 *  currentMembers so the slot shows FULL (red) when capacity is hit.
 * ════════════════════════════════════════════════════════════════════*/
(function () {
  'use strict';
  ...
  window.addEventListener('bmg:paymentConfirmed', function (e) {
    ...
    var memberCount = pending.memberCount || window._poolMemberCount || 1;
    if (!slotId) return;

    var db = window.db;
    if (!db) return;

    /* Delay so app.js transaction (+1) commits first, then we correct */
    setTimeout(async function () {
      try {
        /* Sum memberCount across ALL confirmed bookings for this slot */
        ...
        if (totalMembers === 0) totalMembers = memberCount;
        ...
        console.log('[pool-fix PART6] Slot corrected — ' + totalMembers + '/' + max + (isFull ? ' FULL' : ' available'));

      } catch (err) {
        /* Fallback: simple add of remaining members (app.js added +1 already) */
        try {
          var slotRef2 = db.collection('pool_slots').doc(slotId);
          var d2       = (await slotRef2.get()).data() || {};
          var max2     = d2.maxMembers || 50;
          var newCount = (d2.currentMembers || 0) + Math.max(0, memberCount - 1);
          await slotRef2.update({
            currentMembers: newCount,
            status        : newCount >= max2 ? 'full' : 'available',
            updatedAt     : firebase.firestore.FieldValue.serverTimestamp(),
          });
          console.log('[pool-fix PART6 fallback] currentMembers set to', newCount);
        } catch (e2) {
          console.warn('[pool-fix PART6] fallback error:', e2.message);
        }
      }
    }, 1800);
  });

  console.log('✅ [bmg_swimming_pool_fix.js PART6] Pool slot status fix loaded');
})();