/* combined_patches.js — extracted from sportobook_master.js
 */
...
(function () {
  ...
  /* PATCH 4: Wire pool-book-now-btn price data before booking proceeds.
     Button DOM wiring is now handled centrally by app.js _bmgRewirePoolBookBtn.
     We patch window.handlePoolBookNow to inject correct amount instead. */
  window.addEventListener('bmg:pageShown', function (e) {
    if (!e.detail || e.detail.pageId !== 'pool-page') return;
    setTimeout(function () {
      // Ensure button uses shared rewire (delegates to window.handlePoolBookNow)
      if (typeof window._bmgRewirePoolBookBtn === 'function') {
        window._bmgRewirePoolBookBtn();
      }
      console.log('[pool-price-fix] pool-book-now-btn delegated to _bmgRewirePoolBookBtn');
    }, 200);
  });
...
(function () {
  ...
  window.addEventListener('bmg:paymentConfirmed', async function (e) {
    ...
    // Pool bookings are handled exclusively by finalisePoolBooking in
    // sportobook_combined_patches.js PART3. Skip here to avoid spurious
    // "missing fields" warnings (pool bookings have no groundId).
    var data = e.detail.result || {};
    try {
      var _pendingCheck = JSON.parse(
        sessionStorage.getItem('pendingBooking') ||
        sessionStorage.getItem('pendingCashfreeBooking') || 'null'
      );
      if (_pendingCheck && (_pendingCheck.isPoolBooking || _pendingCheck.poolId)) return;
    } catch (_) {}
    if (data.isPoolBooking || data.poolId) return;
...
(function () {
  ...
  window.addEventListener('bmg:paymentConfirmed', async function (e) {
    ...
    // Pool bookings are handled exclusively by finalisePoolBooking in
    // sportobook_combined_patches.js PART3. Skip here.
    const _data = e.detail.result || {};
    if (_data.isPoolBooking || _data.poolId) return;
    try {
      const _p = JSON.parse(
        sessionStorage.getItem('pendingBooking') ||
        sessionStorage.getItem('pendingCashfreeBooking') || 'null'
      );
      if (_p && (_p.isPoolBooking || _p.poolId)) return;
    } catch (_) {}
  ...
  window.addEventListener('bmg:paymentConfirmed', async function (e) {
    ...
    // Pool bookings don't have a ground 'bookings' doc — skip this repair entirely.
    if (data.isPoolBooking || data.poolId) return;
    ...
    if (!amount) {
      try {
        ...
        if (stored) {
          if (stored.isPoolBooking || stored.poolId) return;
          amount = Number(stored.amount || stored.originalAmount || 0);
        }
      } catch (_) {}
    }
...
(function () {
  ...
  async function computeRealBalance(ownerId) {
    ...
    /* ── Pool bookings — single query on status='confirmed' only ── */
    // FIXED: Previously queried BOTH status='confirmed' AND bookingStatus='confirmed', then
    // relied on Set dedup. Since finalisePoolBooking sets BOTH fields on the same doc,
    // the Set dedup handled it correctly, but the double Firestore query was wasteful.
    // More importantly: if a stale BMG_BOOKING_... doc existed (from the old confirmPoolBooking
    // bug where orderId was used as doc key), it would match bookingStatus='confirmed' but
    // be a DIFFERENT document from the PBK_... doc, bypassing the Set dedup and double-counting.
    // Single query on status='confirmed' is the safe, correct approach.
    const pByStatus = await db.collection('pool_bookings').where('ownerId', '==', ownerId).where('status', '==', 'confirmed').get().catch(() => ({ docs: [] }));
  ...
  window.addEventListener('bmg:paymentConfirmed', async function (e) {
    ...
    // Pool bookings don't have a ground 'bookings' doc — skip this repair.
    const _rd = e.detail.result || {};
    if (_rd.isPoolBooking || _rd.poolId) return;
    try {
      const _ps = JSON.parse(
        sessionStorage.getItem('pendingBooking') ||
        sessionStorage.getItem('pendingCashfreeBooking') || 'null'
      );
      if (_ps && (_ps.isPoolBooking || _ps.poolId)) return;
    } catch (_) {}
