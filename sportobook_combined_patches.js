...
pByStatus.docs.forEach(d => {
  ...
  // CRITICAL FIX:
  // Ignore old duplicate payment docs created with gateway order IDs.
  // Real pool booking docs always use PBK_ ids.
  // Old bug created duplicate BMG_BOOKING_ docs which caused double earnings.
  if (String(d.id || '').startsWith('BMG_BOOKING_')) {
    console.log('[pool-earnings-fix] skipping duplicate gateway doc:', d.id);
    return;
  }
    // Extra protection against duplicate counting
  const uniqueKey = data.bookingId || d.id;
  if (seenPool.has(uniqueKey)) return;
  seenPool.add(uniqueKey);
