  ...
  <div style="padding:14px;">
    ...
    async function fixedOwnerPayouts(container) {
      ...
      try {
        ...
        // ── Total earned ─────────────────────────────────────────
        // Only count bookings NOT yet fully paid out.
        // Bookings marked payout_done are settled and must not appear
        // in Available to Withdraw. Rejected payouts reset booking status
        // back to pending so they re-appear here automatically.
        let totalEarned = 0;
...
(function () {
  ...
  async function computeRealBalance(ownerId) {
    ...
    gSnap.docs.forEach(d => {
      ...
      if (data.payoutStatus === 'payout_done' || data.payoutStatus === 'payout_pending') return;
      const amt  = Number(data.ownerAmount);
      groundEarned += amt > 0 ? amt : Math.floor(Number(data.amount || data.totalAmount || 0) * 0.9);
    });
      ...
      if (!tSnap.empty) {
        ...
        for (let i = 0; i < ids.length; i += 30) {
          ...
          eSnap.docs.forEach(d => {
            ...
            if (e.payoutStatus === 'payout_done' || e.payoutStatus === 'payout_pending') return;
            tournamentEarned += Number(e.ownerAmount) || Math.round(Number(e.amount || 0) * 0.8);
          });
        }
      }
