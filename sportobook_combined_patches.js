/* sportobook_combined_patches.js — extracted from sportobook_master.js
 * NOTE: owners.totalEarnings increment has been REMOVED from this file.
 * Earnings are calculated from bookings.ownerAmount only.
 */
...
(function () {
  ...
  async function finaliseGroundBooking(orderId, data) {
    ...
    /* ── 4. totalEarnings increment REMOVED ─────────────────────────────
     *  Earnings are calculated ONLY from confirmed booking docs
     *  (bookings.ownerAmount + pool_bookings.ownerAmount).
     *  Incrementing owners.totalEarnings here caused double-counting.
     * ─────────────────────────────────────────────────────────────── */
  ...
  async function finalisePoolBooking(orderId, detail) {
    ...
    let bookingDocId = pending.bookingId || pending.orderId || orderId || '';

// CRITICAL FIX:
// Never allow gateway order IDs to become pool booking doc IDs.
// Only PBK_ ids are valid booking documents.
if (String(bookingDocId).startsWith('BMG_BOOKING_')) {
  bookingDocId = pending.originalBookingId || pending.poolBookingId || '';
}
...
(function () {
  ...
  async function confirmPoolBooking(orderId, data) {
    ...
    // CRITICAL FIX: Always use the original PBK_... bookingId as the Firestore doc key.
    // Using orderId (the Cashfree BMG_BOOKING_... gateway ID) would create a SECOND
    // pool_bookings doc separate from the one finalisePoolBooking writes (PBK_...),
    // causing the earnings query to find both docs and count ownerAmount twice.
    const bookingDocId = merged.bookingId || merged.orderId || orderId;

    L('confirmPoolBooking → confirming booking doc only (no slot increment):', { bookingDocId, orderId, slotId, ownerId });

    // Only confirm the pool_bookings doc status.
    // currentMembers update is handled by finalisePoolBooking (PART3 listener).
    try {
      const bookingRef = d.collection('pool_bookings').doc(bookingDocId);
...
(function () {
  ...
  async function computeRealBalance(ownerId) {
    ...
    let poolEarned = 0;
    const seenPoolZero = new Set();
    pByStatus.docs.forEach(d => {
      // Skip stale BMG_BOOKING_... duplicate docs from old payment gateway bug
      if (String(d.id || '').startsWith('BMG_BOOKING_')) return;
      if (seenPoolZero.has(d.id)) return;
      seenPoolZero.add(d.id);
      const data = d.data();
...
pByStatus.docs.forEach(d => {
  ...
  // CRITICAL FIX: skip stale gateway-ID docs (BMG_BOOKING_...) — these are
  // duplicate docs created by an old bug where the Cashfree orderId was used
  // as the Firestore doc key instead of the canonical PBK_... booking ID.
  // Including them causes double-counting of ownerAmount in the earnings total.
  if (String(d.id || '').startsWith('BMG_BOOKING_')) {
    console.log('[pool-earnings-fix] skipping duplicate gateway doc:', d.id);
    return;
  }

  // Deduplicate strictly by Firestore doc ID only.
  // Do NOT use data.bookingId as the uniqueKey — two distinct bookings can
  // legitimately share the same bookingId value if the field was back-filled,
  // which would cause genuine earnings to be silently skipped.
  if (seenPool.has(d.id)) return;
  seenPool.add(d.id);

  if (data.payoutStatus === 'payout_done') return;

  const amt = Number(data.ownerAmount || 0);
  const fallbackAmt = Math.floor(Number(data.amount || 0) * 0.93);

  poolEarned += amt > 0 ? amt : fallbackAmt;
});
  ...
  function buildEarningsRenderer() {
    return async function renderEarningsTabV4(container) {
      ...
      try {
        ...
        // Single query on status='confirmed' only — matches computeRealBalanceV4.
        // Triple-net queries (status + bookingStatus + paymentStatus) caused the same
        // doc to appear 3x since finalisePoolBooking sets ALL three fields on one doc.
        // Even with seenP dedup the triple query was wasteful and error-prone.
        const pS1 = await db.collection('pool_bookings')
          .where('ownerId','==',cu.uid)
          .where('status','==','confirmed')
          .get().catch(()=>({docs:[]}));
        const seenP = new Set();
        pS1.docs.forEach(d => {
          // Skip stale gateway-ID duplicate docs
          if (String(d.id || '').startsWith('BMG_BOOKING_')) return;
