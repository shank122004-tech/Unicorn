...
(function(){
// ─── SLOT WRITE HELPERS (sportobook) ─────────────────────────────────────────
// These write/update slot documents in Firestore after payment events.
// loadSlots() in app.js now owns the onSnapshot listener — we do NOT override it.

// lockBooked: marks a slot as 'booked' in Firestore (creates doc if not yet existing)
async function lockBooked(groundId,date,slotTime,bookingId,userId){
  if(!groundId||!date||!slotTime||!window.db)return;
  var parts=String(slotTime).replace(/\s/g,'').split('-');
  var startTime=parts[0]||'';
  var endTime=parts[1]||'';
  if(!startTime){console.error('[spb] lockBooked: invalid slotTime',slotTime);return;}
  try{
    var FS=_SPB.FS();
    var sd={status:'booked',bookingId:bookingId||'',bookedBy:userId||'',
            bookedAt:FS.serverTimestamp(),lockedBy:null,lockExpiresAt:null,
            lockId:null,updatedAt:FS.serverTimestamp()};
    var slotSnap=await window.db.collection('slots')
      .where('groundId','==',groundId).where('date','==',date)
      .where('startTime','==',startTime).get();
    var batch=window.db.batch();
    if(!slotSnap.empty){
      slotSnap.docs.forEach(function(d){batch.update(d.ref,sd);});
    } else {
      // Slot doc does not exist yet — CREATE it so the onSnapshot picks it up for all users
      var ref=window.db.collection('slots').doc();
      batch.set(ref,Object.assign({groundId:groundId,date:date,
        startTime:startTime,endTime:endTime,slotTime:startTime+'-'+endTime,
        createdAt:FS.serverTimestamp()},sd));
    }
    var lockSnap=await window.db.collection('slot_locks')
      .where('groundId','==',groundId).where('date','==',date)
      .where('startTime','==',startTime).get()
      .catch(function(){return{docs:[]};});
    lockSnap.docs.forEach(function(d){batch.delete(d.ref);});
    await batch.commit();
    console.log('[spb-slots] \u2705 Slot BOOKED:',slotTime,date,'groundId:',groundId);
  }catch(err){console.error('[spb-slots] lockBooked error:',err);}
}

// releaseSlot: releases a locked/pending slot back to available
async function releaseSlot(groundId,date,slotTime,lockId){
  if(!groundId||!date||!slotTime||!window.db)return;
  var startTime=String(slotTime).replace(/\s/g,'').split('-')[0];
  try{
    var FS=_SPB.FS();
    var batch=window.db.batch();
    var slotSnap=await window.db.collection('slots')
      .where('groundId','==',groundId).where('date','==',date)
      .where('startTime','==',startTime).get();
    slotSnap.docs.forEach(function(d){
      var s=d.data();
      if(s.status==='locked'||s.status==='pending')
        batch.update(d.ref,{status:'available',lockedBy:null,lockExpiresAt:null,
          lockId:null,bookingId:null,bookedBy:null,updatedAt:FS.serverTimestamp()});
    });
    if(lockId)batch.delete(window.db.collection('slot_locks').doc(lockId));
    var lockSnap=await window.db.collection('slot_locks')
      .where('groundId','==',groundId).where('date','==',date)
      .where('startTime','==',startTime).get().catch(function(){return{docs:[]};});
    lockSnap.docs.forEach(function(d){batch.delete(d.ref);});
    await batch.commit();
    console.log('[spb-slots] \u2705 Slot released:',slotTime,date);
  }catch(err){console.error('[spb-slots] release error:',err);}
}

// On payment confirmed: write 'booked' to Firestore — onSnapshot propagates to ALL users instantly
window.addEventListener('bmg:paymentConfirmed',async function(e){
  var det=e.detail||{};
  if(det.paymentType!=='booking')return;
  var d=det.result||{};
  if(!d.groundId||!d.date||!d.slotTime)return;
  await lockBooked(d.groundId,d.date,d.slotTime,
    d.bookingId||det.orderId,
    d.userId||(window.currentUser&&window.currentUser.uid));
});

// On payment failed: release the lock
window.addEventListener('bmg:paymentFailed',async function(e){
  var d=e.detail||{};
  var ss=sessionStorage.getItem('slotLock');
  var info=d;
  if(ss){try{info=Object.assign({},JSON.parse(ss),info);}catch(_){}}
  await releaseSlot(info.groundId,info.date,info.slotTime,info.lockId||info.orderId);
  sessionStorage.removeItem('slotLock');
  sessionStorage.removeItem('currentBookingDetails');
  _SPB.toast('Slot has been released. You can try again.','info');
});

// Patch releaseSlotLock to also release sportobook lock records
_SPB.waitFor(function(){return typeof window.releaseSlotLock==='function';},6000).then(function(){
  var _o=window.releaseSlotLock;
  if(_o&&_o._spbP)return;
  window.releaseSlotLock=async function(orderId){
    if(!orderId)return;
    try{if(typeof _o==='function')await _o(orderId);}catch(_){}
    var ss=sessionStorage.getItem('slotLock');
    if(ss){try{var lock=JSON.parse(ss);await releaseSlot(lock.groundId,lock.date,lock.slotTime,orderId);}catch(_){}}
  };
  window.releaseSlotLock._spbP=true;
});

// patchLoadSlots DISABLED — app.js loadSlots() owns the real-time listener.
console.log('[spb-slots] \u2705 Slot write helpers active. loadSlots() managed by app.js.');
})();
