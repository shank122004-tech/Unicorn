...
(function () {
  ...
  async function verifyTournamentQR(qrObject) {
    ...
    try {
      ...
      const html=`<div style="padding:20px;background:#ecfdf5;border-radius:16px;text-align:center;margin-top:12px;"><div style="font-size:48px;margin-bottom:8px;">✅</div><h3 style="color:#059669;margin:0 0 4px;">Tournament Entry Verified!</h3><p style="color:#374151;margin:0 0 14px;font-size:14px;">${window._esc(entry.tournamentName||'Tournament')}</p><div style="background:#fff;border-radius:12px;padding:14px;text-align:left;"><p style="margin:0 0 8px;"><strong>Player:</strong> ${window._esc(entry.userName||'')}</p>${entry.teamName?`<p style="margin:0 0 8px;"><strong>Team:</strong> ${window._esc(entry.teamName)}</p>`:''}<p style="margin:0;"><strong>Sport:</strong> ${window._esc(entry.sport||'')}</p></div></div>`;
      if (resultDiv) resultDiv.innerHTML = html;
      window._bmgToast('✅ Tournament entry verified!','success');
    } catch (err) {
      const html=`<div style="padding:20px;background:#fef2f2;border-radius:16px;text-align:center;margin-top:12px;"><div style="font-size:48px;">❌</div><h3 style="color:#dc2626;margin:0 0 4px;">Verification Failed</h3><p style="color:#374151;font-size:14px;">${window._esc(err.message)}</p></div>`;
      if (resultDiv) resultDiv.innerHTML = html;
  ...
  async function _verifyBookingQRCompact(bookingId) {
    ...
    try {
      ...
      if (!booking) { window._bmgToast('Booking not found','error'); return; }
      const confirmed = booking.bookingStatus==='confirmed'||booking.status==='confirmed'||booking.paymentStatus==='PAID';
      const resultDiv = document.getElementById('professional-qr-result');
      if (confirmed) {
        const html=`<div style="padding:20px;background:#ecfdf5;border-radius:16px;text-align:center;margin-top:12px;"><div style="font-size:48px;">✅</div><h3 style="color:#059669;">Booking Verified!</h3><div style="background:#fff;border-radius:12px;padding:14px;text-align:left;"><p style="margin:0 0 8px;"><strong>Booking ID:</strong> ${window._esc(booking.bookingId||booking.id)}</p><p style="margin:0 0 8px;"><strong>Name:</strong> ${window._esc(booking.userName||'')}</p><p style="margin:0 0 8px;"><strong>Ground:</strong> ${window._esc(booking.groundName||'')}</p><p style="margin:0 0 8px;"><strong>Date:</strong> ${booking.date||''}</p><p style="margin:0 0 8px;"><strong>Time:</strong> ${booking.slotTime||''}</p><p style="margin:0;"><strong>Amount Paid:</strong> ₹${booking.amount||0}</p></div></div>`;
        if (resultDiv) resultDiv.innerHTML = html;
        window._bmgToast('✅ Valid booking confirmed','success');
      } else {
        if (resultDiv) resultDiv.innerHTML=`<div style="padding:20px;background:#fef2f2;border-radius:16px;text-align:center;"><div style="font-size:48px;">❌</div><h3 style="color:#dc2626;">Not Confirmed</h3><p>Status: ${booking.bookingStatus||'unknown'}</p></div>`;
        window._bmgToast('❌ Booking not confirmed','error');
      }
    } catch (err) { window._bmgToast('Verify error: '+err.message,'error'); }
  }
  ...
  /* ── loadUserBookings: add tournament entries ─────────────────────────── */
      ...
      try {
        ...
        const html = entries.map(e => `
          <div class="booking-card status-${e.status||'confirmed'}" style="border-left:4px solid #10b981;margin-bottom:14px;">
            ...
            ${e.status!=='cancelled'?`<button class="auth-btn" onclick="showTournamentEntryPass('${e.id}')" style="margin-top:10px;background:linear-gradient(135deg,#10b981,#059669);">
              <i class="fas fa-qrcode"></i> View Tournament Pass</button>`:''}
          </div>`).join('');
        const empty = container.querySelector('.empty-state');
        if (empty) { container.innerHTML = html; }
        else {
          const hdr = document.createElement('div');
          hdr.style.cssText='margin:18px 0 8px;padding:10px 0;border-top:1px solid #e5e7eb;';
          hdr.innerHTML='<h4 style="color:#374151;font-weight:700;margin:0;">🏆 Tournament Registrations</h4>';
          container.appendChild(hdr);
          const wrap = document.createElement('div'); wrap.innerHTML=html; container.appendChild(wrap);
        }
      } catch (_) {}
    };
  });
...
(function () {
  ...
  window._bmgWaitFor('showEntryPass', () => {
    ...
    window.showEntryPass = async function (bookingId) {
      ...
      window._bmgShowLoading('Generating entry pass…');
      try {
        const db = window.db;
        let booking = null;
